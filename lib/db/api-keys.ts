...
export const getOpenAIApiKey = () => {
  ...
  if (typeof window === 'undefined') {
    // Use Heroku Inference key first, fall back to OpenAI key
    return process.env.INFERENCE_KEY || process.env.OPENAI_API_KEY;
  }
  
  // Check localStorage on client side
  const apiKey = getLocalStorage('openaiApiKey');
  return apiKey || process.env.INFERENCE_KEY || process.env.OPENAI_API_KEY;
};
